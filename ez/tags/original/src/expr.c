...
struct node *term()
{
	...
	switch (t) {
		...
		case CON:
			p = node(O_GVAL, T_INT,
				const(mkint(atoi(lexval.l_str))), NULL);
			t = gtok(0);
			break;
		case FCON:
			p = node(O_GVAL, T_REAL,
				const(mkreal(atof(lexval.l_str))), NULL);
			t = gtok(0);
			break;
		case SCON:
			p = node(O_GVAL, T_STRING,
				const(stralloc(lexval.l_str,lexval.l_length)),
				NULL);
			t = gtok(0);
			break;
		case BCON:
			...
			p = node(O_GVAL, TYPE(v), const(v), NULL);
			t = gtok(0);
			break;
		case PROCEDURE:
			p = node(O_GVAL, T_PROC,
				const(mkval(T_PROC, 0, func())), NULL);
			break;
		case '(':
			t = gtok('\n');
			p = expr1();
		...
		default:
			...
			return (node(O_GVAL, T_INT, const(mkint(0)), NULL));
		}
	while (t == '.' || t == '[' || t == '(')
		switch (t) {
			case '.':
				t = gtok('\n');
				if (t == ID) {
					a = const(stralloc(lexval.l_str, -1));
					t = gtok(0);
					}
				else {
...
/* const - allocate storage for constant v, return virtual address */
int const(v)
struct value v;
{
	int a;
