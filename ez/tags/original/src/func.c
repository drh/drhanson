...
#define	VOID(x)	(x)
/* x_screen - screen(x, y, s) */
static struct value x_screen(n, ap)
int n;
struct value *ap;
{
	char *s;
	struct value sv, xv, yv;
	int i;
	...
	if (n == 0) {
		...
		xv = cvi(*ap++);
		yv = cvi(*ap++);
		move(yv.v_addr-1, xv.v_addr-1);
		if (n == 3) {
			sv = cvstr(*ap++);
			s = SOPENR(sv.v_addr);
			for (i = 0; i < XFIELD(sv); i++)
				addch(SGETC(s));
			SCLOSER(s);
			}
		else
...
struct bivalue bivalues[] = {	/**** do not change position of functions ****/
	...
	"screen",  T_PROC, 0, NULL, x_screen,   {T_VOID, 0},
...
TUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~\177",
	...
	NULL,	   NULL, 0,	NULL,	NULL,	{T_VOID, 0}
};
