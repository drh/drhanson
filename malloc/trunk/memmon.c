...
} *htab[1543];			/* hash table, indexed by block address */
...
static void _free(void *ptr, void *calls[]) {
	...
	if (bp == NULL) {
		...
		fprintf(errfp, "   free(%p) called from:\n", ptr);
		print(calls);
...
static void _malloc(void *ptr, size_t size, void *calls[]) {
	...
	if (show_all_calls) {
		...
		print(calls);
		fprintf(errfp, "   malloc returned %p\n", ptr);
	}
	if (size == 0) {
...
static void _calloc(void *ptr, size_t nobj, size_t size, void *calls[]) {
	...
	if (show_all_calls) {
		...
		print(calls);
		fprintf(errfp, "   calloc returned %p\n", ptr);
	}
	if (nobj == 0 || size == 0) {
...
static void _realloc(void *ptr[], size_t size, void *calls[]) {
	...
	if (show_all_calls) {
		fprintf(errfp, "\n== realloc(%p,%u) called from:\n", ptr[0], size);
		print(calls);
		fprintf(errfp, "   realloc returned %p\n", ptr[1]);
	}
	if (ptr[0] == NULL && size == 0) {
		fprintf(errfp, "\n** realloc'ing a NULL pointer to zero bytes\n");
		fprintf(errfp, "   realloc(%p,%u) called from:\n", ptr[0], size);
		print(calls);
		fprintf(errfp, "   realloc returned %p\n", ptr[1]);
	}
	if (ptr[0] && bp == NULL) {
...
int main(int argc, char *argv[]) {
	...
	if (a_out) {
		...
		sprintf(cmd, "nm -n %s | grep '[tT] ' >%s",
			a_out, temp_file);
		(void)system(cmd);
		stab = Sym_init(temp_file);
	}
	fprintf(errfp, "%s $Revision$\nOptions:", argv[0]);
